...
import type { ContactInfoIconKey } from '../locales/types';
...
export function ContactPage() {
  ...
  const contactIconMap: Record<ContactInfoIconKey, typeof Mail> = {
    email: Mail,
    phone: Phone,
    office: MapPin,
  };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success(contact.toasts.success);
      setFormData({ name: '', email: '', company: '', message: '' });
    } catch (error) {
      console.error('Error submitting form:', error);
      toast.error(contact.toasts.error);
    } finally {
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12">
            {/* Contact Form */}
            <div>
              <h2 className="text-gray-900 mb-6 text-lg font-semibold">{contact.formTitle}</h2>
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <Label htmlFor="name">{contact.formFields.nameLabel}</Label>
                  <Input
                    id="name"
                    type="text"
                    required
                ...
                <div>
                  <Label htmlFor="email">{contact.formFields.emailLabel}</Label>
                  <Input
                    id="email"
                    type="email"
                    required
                ...
                <div>
                  <Label htmlFor="company">{contact.formFields.companyLabel}</Label>
                  <Input
                    id="company"
                    type="text"
                    ...
                    placeholder={contact.formFields.companyPlaceholder}
                    className="mt-1"
                  />
                </div>

                <div>
                  <Label htmlFor="message">{contact.formFields.messageLabel}</Label>
                  <Textarea
                    id="message"
                    required
            ...
            {/* Contact Information */}
            <div>
              <h2 className="text-gray-900 mb-6 text-lg font-semibold">{contact.infoTitle}</h2>
              <p className="text-gray-600 mb-8">
                {contact.infoDescription}
              </p>

              <div className="space-y-6 mb-12">
                {contact.infoItems.map((info, index) => {
              ...
              <div className="rounded-xl overflow-hidden border border-gray-200 h-64 bg-gray-100 flex items-center justify-center">
                <div className="text-center text-gray-500">
                  ...
                  <iframe
                    src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d950.8663084757393!2d105.78417911971191!3d21.030552365590278!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3135ab4c87a16803%3A0x33777b89208b1827!2zVmnhu4d0IMOBIFRvd2VyLCAwOSBEdXkgVMOibiwgROG7i2NoIFbhu41uZyBI4bqtdSwgQ-G6p3UgR2nhuqV5LCBIw6AgTuG7mWkgMTAwMDAwLCBWaWV0bmFt!5e0!3m2!1sen!2ssg!4v1761330009794!5m2!1sen!2ssg"
                    width="600"
                    height="450"
                    loading="lazy"
                    referrerPolicy="no-referrer-when-downgrade"
                    title={contact.mapDescription}
                  ></iframe>
                  <p>{contact.mapLabel}</p>
                </div>
              </div>
            </div>
          </div>
